...
const fire = (event) => {
  ...
  if (target.classList.length > 0 || target.tagName !== 'TD' || !game.shipCount) {
    return;
  }
  show.miss(target);
      ...
      if (life < 0) {
        ...
        if (!game.shipCount) {
          ...
          if (play.shot < play.record || play.record === 0) {
            localStorage.setItem('battleShipRecord', play.shot);
